...
def load_ribosomal_proteins_from_db(db_path: Path) -> pl.DataFrame:
    ...
    db_df = (
        ...
        .filter(
            ...
            & (pl.col("protein").str.len_chars() <= const.HMM_MAX_SPROT_LENGTH)
            & (
                ~pl.col("product")
                .str.to_lowercase()
                .is_in(pl.Series("hypothetical_product_names", [term for term in const.HYPOTHETICAL_PRODUCT_NAMES]))
            )
        )
        ...
        .with_columns(
            ...
            .str.replace(r"\+|%", " ")
            .str.replace(r"\+|%", " ")
            .str.replace(r"ssu", "30s")
            .str.replace(r"lsu", "50s")
            .str.replace("subunit ", "")
        )
        .with_columns(
            pl.when(pl.col("product").str.len_chars() > 0).then(pl.lit(1)).otherwise(pl.lit(0)).alias("ribosomal")
        )
    )

    return db_df.rename({"protein": "query"}).collect()
...
def export_ribosomal_clusters(graph_df: pl.DataFrame, inflations: list[str], outpath: Path) -> pl.DataFrame:
    ...
    coverage_df: pl.DataFrame = (
        graph_df.lazy()
        .filter(pl.col("ribosomal") == 1)
        .group_by(inflations[0], "product")
        .count()
        .group_by("product")
        .agg((pl.col("count") / pl.col("count").sum() * 100).round(2).sort().alias(f"coverage-{inflations[0]}"))
        .with_columns(pl.col(f"coverage-{inflations[0]}").cast(pl.List(pl.Utf8)).list.join(", "))
        .sort("product")
        .collect()
    )
    ...
    for inflation in inflations[1:]:
        lazy_frames.append(
            graph_df.lazy()
            .filter(pl.col("ribosomal") == 1)
            .group_by(inflation, "product")
            .count()
            .group_by("product")
            .agg((pl.col("count") / pl.col("count").sum() * 100).round(2).sort().alias(f"coverage-{inflation}"))
            .with_columns(pl.col(f"coverage-{inflation}").cast(pl.List(pl.Utf8)).list.join(", "))
            .sort("product")
        )
